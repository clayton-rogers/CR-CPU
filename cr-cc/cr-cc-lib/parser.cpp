...
static std::map<TokenType, RuleList> C_TOP_LEVEL = {
...
static std::map<TokenType, RuleList> C_FUNCTION = {
	{TokenType::function,
		{
...
static std::map<TokenType, RuleList> C_STATEMENT = {
	{TokenType::statement,
		{
			{{TokenType::compound_statement}, {}},
...
static std::map<TokenType, RuleList> C_EXPRESSION = {
...
static const std::map<TokenType, RuleList>* get_c_grammar()
{
	static std::map<TokenType, RuleList> C_GRAMMAR;
	static bool needs_init = true;

	if (needs_init) {
		C_GRAMMAR.merge(C_DECLARATION);
		C_GRAMMAR.merge(C_EXPRESSION);
		C_GRAMMAR.merge(C_FUNCTION);
		C_GRAMMAR.merge(C_STATEMENT);
		C_GRAMMAR.merge(C_TOP_LEVEL);
		needs_init = false;
	}
...
static bool is_token_terminal(TokenType token)
{
	...
	return get_c_grammar()->count(token) != 1;
}
...
static int parse_node(ParseNode* node, const TokenList& token_list, int offset)
{
	if (is_token_terminal(node->token.token_type)) {
		...
		RuleList rule_list = get_c_grammar()->at(node->token.token_type);
