...
namespace AST
{
	...
	std::string Assignment_Expression::generate_code() const
	{
		...
		if (is_pointer) {
			// The value is in ra, compute the address to store through
			// and leave the value in ra
			ss << scope->push_reg("ra") + " # push rhs while calc'ing ptr addr\n";
			ss << pointer_rhs->generate_code();
			ss << "mov rp, ra # move ptr addr into rp\n";
			ss << scope->pop_reg("ra") + " # get back rhs\n";
